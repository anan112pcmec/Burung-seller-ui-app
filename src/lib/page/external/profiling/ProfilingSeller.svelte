<script lang="ts">
    ...
    // ///////////////////////////////////////////////////////////////////////
    // State — Foto Profil, Banner, & Toko Fisik
    // ///////////////////////////////////////////////////////////////////////

    let statusFoto = $state<StatusFoto>('terunggah');
    let fotoProfilUrl = $state('https://picsum.photos/seed/toko-ahmad-fauzan/200/200');

    // State Banner (Hanya 1)
    let statusBanner = $state<StatusFoto>('terunggah');
    let bannerUrl = $state('https://picsum.photos/seed/banner-toko/800/300');

    // State Toko Fisik (Lebih dari satu)
    let listFotoFisik = $state<FotoFisik[]>([
        { id: '1', url: 'https://picsum.photos/seed/fisik-1/300/200' },
        { id: '2', url: 'https://picsum.photos/seed/fisik-2/300/200' }
    ]);
    ...
    function ubahBanner(e: Event) {
        const target = e.target as HTMLInputElement;
        const file = target.files?.[0];
        if (!file) return;

        statusBanner = 'mengunggah';
        const previewSementara = URL.createObjectURL(file);

        setTimeout(() => {
            bannerUrl = previewSementara;
            statusBanner = 'terunggah';
        }, 700);
    }

    function tambahFotoFisik(e: Event) {
        const target = e.target as HTMLInputElement;
        const files = target.files;
        if (!files || files.length === 0) return;

        // Simulasi multiple upload
        Array.from(files).forEach((file) => {
            const previewSementara = URL.createObjectURL(file);
            listFotoFisik = [...listFotoFisik, { id: Math.random().toString(), url: previewSementara }];
        });
    ...
    function hapusFotoFisik(id: string) {
        listFotoFisik = listFotoFisik.filter((f) => f.id !== id);
    }
    ...
    let itemKelengkapan = $derived([
        ...
        { label: 'Foto banner toko', selesai: statusBanner === 'terunggah' },
...
<section id="profiling" class="w-full bg-white p-4 sm:p-6 lg:p-8 text-slate-950 pb-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-7 flex flex-col gap-6">
            ...
            <!-- FOTO BANNER SELLER (Hanya 1) -->
            <div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
                <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] text-slate-950/40 uppercase font-mono">
                    Foto Banner Toko
                </span>
                <p class="text-[10px] text-zinc-400 mt-1 leading-relaxed">
                    Banner utama yang tampil di bagian atas halaman toko kamu.
                </p>
                <div class="mt-3 flex flex-col gap-3">
                    <div class="relative w-full h-32 sm:h-40 rounded overflow-hidden bg-zinc-100 border border-zinc-200">
                        {#if statusBanner === 'mengunggah'}
                            <div class="absolute inset-0 flex items-center justify-center bg-white/70">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-zinc-400 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
                                </svg>
                        ...
                        {:else}
                            <img src={bannerUrl} alt="Banner toko" class="w-full h-full object-cover" />
                        {/if}
                    </div>
                    <div>
                        <label class="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-950 border border-zinc-300 rounded px-3 py-1.5 cursor-pointer hover:bg-zinc-50 transition-colors">
                            Ganti Banner (1 Foto)
                            <input type="file" accept="image/*" class="hidden" onchange={ubahBanner} />
                        </label>
                    </div>
                </div>
            </div>

            <!-- FOTO TOKO FISIK (Lebih dari satu) -->
			...
			<div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
                <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] text-slate-950/40 uppercase font-mono">
                    Foto Toko Fisik / Gudang
                </span>
                <p class="text-[10px] text-zinc-400 mt-1 leading-relaxed">
                    Tambahkan beberapa foto sudut toko atau gudang kamu untuk menambah kepercayaan pembeli.
                </p>
                <div class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {#each listFotoFisik as foto (foto.id)}
                        <div class="relative group h-24 rounded bg-zinc-100 border border-zinc-200 overflow-hidden">
                            <img src={foto.url} alt="Foto toko fisik" class="w-full h-full object-cover" />
                            <button
                                type="button"
                                onclick={() => hapusFotoFisik(foto.id)}
                                class="absolute top-1.5 right-1.5 bg-slate-950/80 text-white p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity text-[9px]"
                                title="Hapus foto"
                            >
                    ...
                    {/each}
                    <label class="flex flex-col items-center justify-center h-24 rounded border border-dashed border-zinc-300 cursor-pointer hover:bg-zinc-50 transition-colors text-zinc-400 hover:text-zinc-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                            <path d="M12 5v14M5 12h14"/>
                        </svg>
                        <span class="text-[9px] font-medium uppercase tracking-wider">Tambah Foto</span>
                        <input type="file" accept="image/*" multiple class="hidden" onchange={tambahFotoFisik} />
                    </label>
                </div>
            </div>
