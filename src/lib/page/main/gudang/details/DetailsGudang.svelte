<script lang="ts">
	...
	// ///////////////////////////////////////////////////////////////////////
	// Detail Alamat Gudang (Read-Only Data)
	// ///////////////////////////////////////////////////////////////////////
...
<section id="details-gudang" class="w-full bg-white text-slate-950">
	...
	<div class="px-4 sm:px-6 lg:px-8 py-6">
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-5">
			<!-- KOLOM KIRI — Informasi Alamat (Display Text Only) -->
			<div class="lg:col-span-8 flex flex-col gap-4">
				<div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
					...
					<div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div class="flex flex-col gap-1 sm:col-span-2">
							<span class="text-[10px] font-medium text-slate-500">Nama Panggilan Alamat</span>
							<p class="text-xs font-semibold text-slate-900">{form.panggilanAlamat}</p>
						</div>

						<div class="flex flex-col gap-1">
							<span class="text-[10px] font-medium text-slate-500">Nomor Telepon</span>
							<p class="text-xs font-mono text-slate-900">{form.nomorTelepon}</p>
						</div>

						<div class="flex flex-col gap-1">
							<span class="text-[10px] font-medium text-slate-500">Kode Negara</span>
							<p class="text-xs font-mono uppercase text-slate-900">{form.kodeNegara}</p>
						</div>
					</div>
				</div>
				...
				<div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
					...
					<div class="mt-3 flex flex-col gap-4">
						<div class="flex flex-col gap-1">
							<span class="text-[10px] font-medium text-slate-500">Nama Alamat / Jalan</span>
							<p class="text-xs text-slate-900 leading-relaxed">{form.namaAlamat}</p>
						</div>

						<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
							<div class="flex flex-col gap-1">
								<span class="text-[10px] font-medium text-slate-500">Provinsi</span>
								<p class="text-xs text-slate-900">{form.provinsi}</p>
							</div>

							<div class="flex flex-col gap-1">
								<span class="text-[10px] font-medium text-slate-500">Kota</span>
								<p class="text-xs text-slate-900">{form.kota}</p>
							</div>

							<div class="flex flex-col gap-1">
								<span class="text-[10px] font-medium text-slate-500">Kode Pos</span>
								<p class="text-xs font-mono text-slate-900">{form.kodePos}</p>
							</div>
						</div>

						<div class="flex flex-col gap-1">
							<span class="text-[10px] font-medium text-slate-500">Deskripsi / Catatan Akses</span>
							<p class="text-xs text-slate-900 leading-relaxed">{form.deskripsi}</p>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="lg:col-span-4 flex flex-col gap-4">
				<div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
					...
					<div class="mt-3 grid grid-cols-2 gap-3">
						<div class="flex flex-col gap-1">
							<span class="text-[10px] font-medium text-slate-500">Latitude</span>
							<p class="text-xs font-mono text-slate-900">{form.latitude}</p>
						</div>
						<div class="flex flex-col gap-1">
							<span class="text-[10px] font-medium text-slate-500">Longitude</span>
							<p class="text-xs font-mono text-slate-900">{form.longitude}</p>
						</div>
					</div>
					...
					<a
						...
						class="mt-4 flex items-center justify-center gap-1.5 w-full border border-zinc-300 rounded-md px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-700 hover:bg-zinc-50 transition-colors"
					>
					...
					<p class="text-[9px] text-zinc-400 mt-2 leading-relaxed">
						Koordinat ini dipakai sistem untuk menghitung jarak tempuh & estimasi ongkir pada transaksi yang dikirim dari gudang ini.
					</p>
				</div>
		...
		<div class="mt-10">
			<div class="flex items-center justify-between mb-3">
				...
				<div class="flex items-center gap-2">
					<button
						class="text-[9px] sm:text-[10px] font-medium uppercase tracking-wider border rounded-xs px-3 py-1.5 border-slate-900 bg-slate-900 text-white hover:bg-slate-800 transition-colors shrink-0"
					>
						+ Tambah Barang
					</button>
				</div>
