<script lang="ts">
    ...
    let statusPengajuan = $state<StatusPengajuan>('Ditolak');
    ...
    const fields = [
        { key: 'namaPerusahaan', label: 'Nama Perusahaan', placeholder: 'PT Kreasi Merek Indonesia', required: true },
        { key: 'negaraAsal', label: 'Negara Asal Brand', placeholder: 'Indonesia', required: true },
        { key: 'lembagaPendaftaran', label: 'Lembaga Pendaftaran Merek', placeholder: 'DJKI Kemenkumham', required: true },
        { key: 'nomorPendaftaranMerek', label: 'Nomor Pendaftaran Merek', placeholder: 'IDM000123456', required: true },
        { key: 'nib', label: 'Nomor Induk Berusaha (NIB)', placeholder: '9876543210987', required: true },
        { key: 'npwp', label: 'NPWP Perusahaan', placeholder: '02.345.678.9-012.000', required: true }
    ];

    const docs = [
        { key: 'sertifikatMerek', label: 'Sertifikat Merek', hint: 'PDF, dari lembaga pendaftaran', accept: '.pdf', required: true },
        { key: 'dokumenPerwakilan', label: 'Dokumen Surat Kuasa / Perwakilan', hint: 'PDF, wajib jika bukan pemilik langsung', accept: '.pdf', required: true },
        { key: 'logoFoto', label: 'Logo Brand', hint: 'PNG transparan, resolusi tinggi', accept: '.png', required: true },
        ...
        { key: 'suratKerjasama', label: 'Surat Kerjasama', hint: 'PDF, opsional jika ada perjanjian', accept: '.pdf', required: false }
    ];

    // Derived States untuk Checklist & Ring Progress
    ...
    let itemsKelengkapan = $derived([
        { label: 'Informasi Legal Perusahaan', status: dataFormLengkap },
        { label: 'Nomor HKI & Registrasi Merek', status: Boolean(form.nomorPendaftaranMerek && form.lembagaPendaftaran) },
        { label: 'Dokumen Legalitas Wajib (5 Dokumen)', status: dokumenWajibLengkap },
        { label: 'Logo Brand Terunggah', status: dokumen.logoFoto?.status === 'terunggah' }
    ]);
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start p-5">
    <!-- KOLOM KIRI: INPUT FORM & UPLOAD (Span 7) -->
    <div class="lg:col-span-7 space-y-6">
        ...
        <div class="border rounded-sm p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 {statusClass(statusPengajuan)}">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-wider">Status: {statusPengajuan}</p>
                    ...
                    {:else if statusPengajuan === 'Pending'}
                        <p class="text-[11px] mt-0.5 leading-relaxed opacity-80">Pengajuan brand sedang ditinjau oleh tim legal kami.</p>
                    {:else if statusPengajuan === 'Disetujui'}
                        <p class="text-[11px] mt-0.5 leading-relaxed opacity-80">Badge Official Store / Brand telah aktif.</p>
                    {/if}
                </div>
            </div>
        </div>

        <!-- Section 1: Form Identitas & Kepemilikan -->
        <div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
            <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] text-slate-950/40 uppercase font-mono">
                01. Detail Kepemilikan & Hak Merek
            </span>
        ...
        <div class="border border-zinc-800/20 rounded-sm p-4 sm:p-5">
            <span class="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] text-slate-950/40 uppercase font-mono">
                02. Upload Dokumen HKI & Legalitas
            </span>
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            >
                {statusPengajuan === 'Belum Diajukan' ? 'Ajukan Verifikasi Brand' : 'Simpan & Ajukan Ulang'}
            </button>
        </div>
    </div>

    <!-- KOLOM KANAN: LIVE PREVIEW & PROGRESS OVERVIEW (Span 5 - Sticky) -->
    <div class="lg:col-span-5 lg:sticky lg:top-6 space-y-4">
        
        <!-- Live Card Preview (Official Brand Badge) -->
        <div class="border border-zinc-800/20 rounded-sm p-4 bg-zinc-900 text-white relative overflow-hidden shadow-sm">
            ...
            <div class="flex items-center justify-between border-b border-zinc-800 pb-3 mb-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] font-mono tracking-widest text-zinc-400 uppercase">Brand Certificate Preview</span>
                </div>
                <span class="text-[9px] font-mono bg-zinc-800 text-zinc-300 px-2 py-0.5 rounded border border-zinc-700">
                    {form.negaraAsal || 'INDONESIA'}
                </span>
            </div>

            <div class="space-y-3">
                <div>
                    <p class="text-[10px] font-mono text-zinc-500 uppercase">Nama Perusahaan / Entitas</p>
                    <p class="text-sm font-semibold text-zinc-100 truncate">{form.namaPerusahaan || '—'}</p>
                </div>

                <div class="grid grid-cols-2 gap-2 text-xs font-mono">
                    <div>
                        <p class="text-[9px] text-zinc-500 uppercase">Lembaga HKI</p>
                        <p class="text-[11px] text-zinc-300 truncate">{form.lembagaPendaftaran || '—'}</p>
                    </div>
                    <div>
                        <p class="text-[9px] text-zinc-500 uppercase">No. Pendaftaran</p>
                        <p class="text-[11px] text-teal-400 font-bold truncate">{form.nomorPendaftaranMerek || '—'}</p>
                    </div>
                </div>

                <div class="pt-2 border-t border-zinc-800 grid grid-cols-2 gap-2 text-[10px] font-mono text-zinc-400">
                    <div>NIB: <span class="text-zinc-200">{form.nib || '—'}</span></div>
                    <div>NPWP: <span class="text-zinc-200">{form.npwp || '—'}</span></div>
                </div>
            </div>
        </div>
        ...
        <div class="border border-zinc-800/20 rounded-sm p-4 bg-white">
            <div class="flex items-center justify-between pb-3 border-b border-zinc-100 mb-3">
                <div>
                    <p class="text-[9px] font-bold tracking-[0.15em] text-slate-950/40 uppercase font-mono">Status Kelengkapan</p>
